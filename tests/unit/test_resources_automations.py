...
class TestCheckIfAutomationExists:
    ...
    async def test_check_if_automation_exists_returns_true(self):
        ...
        mock_client = MagicMock()
        mock_ops_mgr = MagicMock()
        mock_ops_mgr.find_automations = AsyncMock(
            return_value=[{"_id": "1", "name": "TestAutomation"}]
        )
        mock_client.operations_manager = mock_ops_mgr
...
class TestGetAutomationIdForName:
    ...
    async def test_get_automation_id_for_name_found(self):
        ...
        mock_ops_mgr = MagicMock()
        mock_ops_mgr.find_automations = AsyncMock(
            return_value=[{"_id": "auto123", "name": "TargetAutomation"}]
        )
        mock_client.operations_manager = mock_ops_mgr
...
class TestEnsureAutomationIsNew:
    ...
    async def test_ensure_automation_is_new_raises_error(self):
        ...
        mock_ops_mgr = MagicMock()
        mock_ops_mgr.find_automations = AsyncMock(
            return_value=[{"_id": "auto1", "name": "ExistingAutomation"}]
        )
        mock_client.operations_manager = mock_ops_mgr
...
class TestImporter:
    ...
    async def test_importer_success(self):
        ...
        mock_ops_mgr.import_automation = AsyncMock(
            return_value={"_id": "new_auto", "name": "TestAutomation"}
        )

        # Mock get_groups for GBAC
        mock_auth.get_groups = AsyncMock(return_value=[{"_id": "1", "name": "Admins"}])
        ...
        automation = {
            ...
            "gbac": {"read": [{"name": "Admins"}], "write": [{"name": "Admins"}]},
        }
    ...
    async def test_importer_validates_component_type(self):
        ...
        automation = {"name": "TestAutomation", "componentType": "jobs"}
    ...
    async def test_importer_checks_for_duplicates(self):
        ...
        mock_ops_mgr = MagicMock()
        mock_ops_mgr.find_automations = AsyncMock(
            return_value=[{"_id": "existing", "name": "TestAutomation"}]
        )
        mock_client.operations_manager = mock_ops_mgr
    ...
    async def test_importer_doesnt_mutate_input(self):
        ...
        await resource.importer(
            original_automation, write_groups=["Admins"], preserve_write_groups=False
        )
    ...
    async def test_importer_with_preserve_groups_false(self):
        ...
        mock_auth.get_groups = AsyncMock(
            return_value=[{"_id": "1", "name": "NewGroup"}]
        )
        ...
        automation = {
            "name": "TestAutomation",
            "componentType": "workflows",
            "gbac": {"read": [{"name": "OldGroup"}], "write": [{"name": "OldGroup"}]},
        }
...
class TestDelete:
    ...
    async def test_delete_success(self):
        ...
        mock_ops_mgr.delete_automation = AsyncMock(
...
class TestProcessGbac:
    ...
    async def test_process_gbac_validates_missing_groups(self):
        ...
        mock_auth = MagicMock()
        mock_auth.get_groups = AsyncMock(
            return_value=[{"_id": "1", "name": "ExistingGroup"}]
        )
        mock_client.authorization = mock_auth

        resource = Resource(mock_client)

        gbac = {"read": [{"name": "MissingGroup"}], "write": []}
...
class TestSetGbac:
    ...
    async def test_set_gbac_updates_automation_gbac(self):
        ...
        automation = {
            "_id": "auto123",
            "name": "TestAutomation",
            "gbac": {"read": [{"_id": "1", "name": "Group1"}], "write": []},
        }
        ...
        mock_ops_mgr.update_automation = AsyncMock(
            return_value={"_id": "auto123", "name": "TestAutomation"}
        )
    ...
    async def test_set_gbac_raises_for_nonexistent_automation(self):
        ...
        with pytest.raises(exceptions.AsyncPlatformError) as exc_info:
            await resource.set_gbac("NonExistentAutomation", read_groups=["Group1"])
    ...
    async def test_set_gbac_raises_for_missing_groups(self):
        ...
        resource = Resource(mock_client)
        resource.get_groups = AsyncMock(
            return_value={"Group1": {"_id": "1", "name": "Group1"}}
        )

        with pytest.raises(exceptions.AsyncPlatformError) as exc_info:
            await resource.set_gbac("TestAutomation", read_groups=["MissingGroup"])
    ...
    async def test_set_gbac_validates_preserved_groups_exist(self):
        ...
        automation = {
            "_id": "auto123",
            "name": "TestAutomation",
            "gbac": {"read": [{"_id": "1", "name": "DeletedGroup"}], "write": []},
        }
        mock_ops_mgr.find_automations = AsyncMock(return_value=[automation])
        mock_ops_mgr.get_automation = AsyncMock(return_value=automation)
