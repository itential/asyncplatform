...
class TestOperationsManagerFindAutomations:
    ...
    @pytest.mark.asyncio
    @patch.object(Service, "get")
    async def test_find_automations_returns_empty_list_when_no_automations(self, mock_get):
    ...
    async def test_find_automations_handles_pagination(self, mock_get):
        ...
        page2 = {
            "metadata": {"total": 250},
            "data": [{"_id": f"{i}", "name": f"Automation{i}"} for i in range(100, 200)]
        }

        page3 = {
            "metadata": {"total": 250},
            "data": [{"_id": f"{i}", "name": f"Automation{i}"} for i in range(200, 250)]
        }
