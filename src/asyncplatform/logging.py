...
r"""Comprehensive logging system for the asyncplatform SDK.
...
def _get_loggers() -> set[logging.Logger]:
    ...
    Retrieves loggers that belong to the application and its dependencies
    (ipsdk). Results are cached to improve performance on subsequent calls.
...
def set_level(lvl: int | str, *, propagate: bool = False) -> None:
    ...
    if isinstance(lvl, str):
        if lvl == "NONE":
            ...
            msg = f"Invalid level string: {lvl}. Only 'NONE' is supported as a string."
            raise TypeError(msg)
