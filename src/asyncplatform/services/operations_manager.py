...
class Service(ServiceBase):
    ...
    async def find_automations(
        ...
        res = await self.get("/operations-manager/automations", params=params)
        ...
        for skip in range(limit, total, limit):
            ...
            tasks.append(
                self.get("/operations-manager/automations", params=page_params)
            )
    ...
    async def import_automation(self, automation: Mapping[str, Any]) -> dict[str, Any]:
        ...
        """
        res = await self.put(
            "/operations-manager/automations", json={"automations": [automation]}
        )
    ...
    async def delete_automation(self, object_id: str) -> dict[str, Any]:
        ...
        """
        res = await self.delete(f"/operations-manager/automations/{object_id}")
    ...
    @logging.trace
    async def update_automation(self, automation: dict[str, Any]) -> dict[str, Any]:
        ...
        res = await self.patch(
            f"/operations-manager/automations/{object_id}", json=automation
        )
        json_data = res.json()
        logging.info(json_data["message"])
        return json_data["data"]
