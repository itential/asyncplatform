...
class Resource(ResourceBase):
    ...
    async def _validate_gbac(
        ...
        return [group for group in groups_to_check if group["name"] not in source_names]
    ...
    async def _is_workflow(self, automation: dict[str, Any]) -> None:
        ...
        if component_type != "workflows":
            msg = (
                f"Invalid automation type, expected `workflows`, got `{component_type}`"
            )
    ...
    async def _ensure_automation_is_new(self, name: str) -> None:
        ...
        if await self._check_if_automation_exists(name):
            raise exceptions.AsyncPlatformError(f"Automation `{name}` already exists")
    ...
    async def get_automation_by_name(self, name: str) -> dict[str, Any] | None:
        ...
        automations = await self.operations_manager.find_automations(name=name)
        return next(
            (automation for automation in automations if automation["name"] == name),
            None,
        )
    ...
    async def importer(
        ...
        *,
        ...
        logging.info(
            f"Successfully imported automation {automation_name} (id: {automation_id})"
        )
